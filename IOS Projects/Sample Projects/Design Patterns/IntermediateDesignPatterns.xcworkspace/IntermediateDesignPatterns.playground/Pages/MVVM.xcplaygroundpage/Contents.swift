...
public class PetViewModel {
    private let pet: Pet
    private let callendar: Calendar
    
    public init( pet: Pet ){
        self.pet = pet
        self.callendar = Calendar(identifier: .gregorian)
    }
    ...
    public var ageText: String {
        let today = callendar.startOfDay(for: Date())
        let birthday = callendar.startOfDay(for: pet.birthday)
        let components = callendar.dateComponents([.year], from: birthday, to: today)
